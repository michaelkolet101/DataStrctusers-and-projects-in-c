...
static void FillArrayWithRand(int *arr_, size_t size_arr_);
...
static void FillArrayWithRand(int *arr_, size_t size_arr_)
{
	...
	srand(time(NULL)); /* set different starting point for rand every time */
	
	for(i = 0; i < size_arr_; ++i) /* fill in randomized numbers */
	{
		*arr_ = rand();
		++arr_;
	}
